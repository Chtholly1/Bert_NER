...
class NERDatasetSplit(Dataset):
    def __init__(self, label_2_ids, idx_list, input_ids, attention_mask, token_type_ids, labels):
        self.label_2_ids = label_2_ids
        self.input_ids = np.array(input_ids)[idx_list]
        self.attention_mask = np.array(attention_mask)[idx_list]
        self.token_type_ids = np.array(token_type_ids)[idx_list]
    ...
    def __getitem__(self, idx):
        ...
        for idx, label in enumerate(new_labels):
            ...
            if label.find('B') >= 0:
                start_ids[idx] = 1
                starts.append(idx)
            if label == 'O' or label.find('B')>=0:
                if idx>0 and new_labels[idx-1] != 'O':
                    end_ids[idx-1] = 1
                    ends.append(idx-1)
            if idx == MAX_LEN-1:
                if label != 'O':
        ...
        match_labels = t.zeros([MAX_LEN, MAX_LEN], dtype=t.long)
        for start, end in zip(starts, ends):
            match_labels[start, end] = 1
        #print("label_ids:", label_ids)
        #print("start_ids:", start_ids)
        #print("endle_ids:", end_ids)
        return t.tensor(token_ids, dtype=t.long), t.tensor(att_mask, dtype=t.long), t.tensor(token_type_ids, dtype=t.long), \
               t.tensor(label_ids, dtype=t.long), t.tensor(start_ids, dtype=t.long), t.tensor(end_ids, dtype=t.long), match_labels
