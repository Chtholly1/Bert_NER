...
class AlBertMRCNERModel(nn.Module):
    def __init__(self, config):
        super(AlBertMRCNERModel, self).__init__()
        self.bert = AlbertModel.from_pretrained(config.model_name)  # /bert_pretrain
        self.classifier1 = nn.Linear(config.embedding_size, 1)
        self.classifier2 = nn.Linear(config.embedding_size, 1)
        self.span_embedding = MultiNonLinearClassifier(config.embedding_size * 2, 1, 0.1,
    ...
    def forward(self, input_ids, att_mask, token_type_ids, label_ids, start_ids, end_ids):
        last_hidden_state, pool_output = self.bert(input_ids=input_ids, attention_mask=att_mask, token_type_ids = token_type_ids)[:2]
        seq_len = last_hidden_state.size(1)

        start_logits = self.classifier1(last_hidden_state).squeeze(-1)
        end_logits = self.classifier2(last_hidden_state).squeeze(-1)

        start_extend = last_hidden_state.unsqueeze(2).expand(-1, -1, seq_len, -1)
        # [batch, seq_len, seq_len, hidden]
        end_extend = last_hidden_state.unsqueeze(1).expand(-1, seq_len, -1, -1)
        # [batch, seq_len, seq_len, hidden*2]
        span_matrix = t.cat([start_extend, end_extend], 3)
...
class AlBertLSTMModel(nn.Module):
    ...
    def forward(self, input_ids, att_mask, token_type_ids, label_ids):
        hidden_states, pooled_output = self.bert(input_ids=input_ids, attention_mask=att_mask, token_type_ids = token_type_ids)[:2]
